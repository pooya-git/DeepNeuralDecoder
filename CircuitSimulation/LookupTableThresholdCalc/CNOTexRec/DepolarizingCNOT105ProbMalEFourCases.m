...
CFull = [0,0,0,0,0,1000,1006,0,1010,0,0,0,0,0,1000,1006,0;
 ...
 parfor i = 1:18
     ...
     errVecStringFull = 'errVecFull';
     str_errVecFull = strcat(errVecStringFull,str_errRate);
     ...
     if (~isempty(eOIndex)) && (~isempty(ePlusIndex))
         errorVecMatFull = depolarizingGenerator(eO,eOIndex,ePlus,ePlusIndex,errRate,numIterations,CFull,n,lookUpPlus,lookUpO);
         parsaveErrorVec(str_errVecFull,errorVecMatFull);
     end
 end
...
function Output = depolarizingGenerator(eO,eOIndex,ePlus,ePlusIndex,errRate,numIterations,CFull,n,lookUpPlus,lookUpO)
...
for ii = 1:numIterations
    ...
    CFullMat(1,:) = mod(Cfinal(1,:) + cxRow18 + equivalenceTable(mod(cxRow18 + cxRow34,2)),2);    
    CFullMat(2,:) = mod(Cfinal(2,:) + czRow17 + czRow20 + equivalenceTable(mod(czRow17 + czRow20 + czRow33,2)),2);
    CFullMat(3,:) = mod(Cfinal(3,:) + cxRow18 + cxRow19 + equivalenceTable(mod(cxRow18 + cxRow19 + cxRow35,2)),2);    
    CFullMat(4,:) = mod(Cfinal(4,:) + czRow20 + equivalenceTable(mod(czRow20+ czRow36,2)),2);
    
    ef1Full = mod(sum(mod(CFullMat(1,:) + equivalenceTable(CFullMat(1,:)),2)),2);
    ef2Full = mod(sum(mod(CFullMat(2,:) + equivalenceTable(CFullMat(2,:)),2)),2);   
    ef3Full = mod(sum(mod(CFullMat(3,:) + equivalenceTable(CFullMat(3,:)),2)),2);
    ef4Full = mod(sum(mod(CFullMat(4,:) + equivalenceTable(CFullMat(4,:)),2)),2);
